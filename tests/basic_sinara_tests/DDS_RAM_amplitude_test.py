...
"""
This is to simulate the situation in our experiment:
1- Do feedback out of RAM.
2- FORT in RAM mode, pulses a few gaussian, for example.
3- Excit RAM for feedback.

repeate the cycle and make sure you can change the amplitude and frequency of different channels while out of RAM mode using
self.dds.set(frequency=f, amplitude=a). This is how it is done in aom_feedback.
...
class DDS_RAM_amplitude_test(EnvExperiment):
    ...
    def run(self):
        ...
        self.dds2.cpld.init()
        self.dds2.init()
        self.dds2.set(frequency=10 * MHz, amplitude=0.1)
        self.dds2.set_att(0.0)
        self.dds2.sw.off()
