...
@kernel
def load_FORT_until_atom_optimizer(self):
    """
    To be used in GVO to optimize atom loading time.

    Similar to load_MOT_and_FORT_until_atom(self), but does not try to load an atom forever. It stops after a few trials.
    Otherwise, we cannot use this function to optimize atom loading time; with wrong shims settings, for example,
    it keeps trying and never finishes the iteration.
...
def atom_loading_2_experiment(self):
    ...
    while self.measurement < self.n_measurements:
        ...
        # load_FORT_until_atom_optimizer(self) ### use only with GVO to optimize loading time
...
def atom_loading_optimizer_experiment(self):
    """
    Simple atom loading experiment based on load_FORT_until_atom_optimizer to optimize atom loading time.
    Use this in GVO aith atom_loading_time_cost. This measures the atom loading time based on RO1 rather in FORT loading.
