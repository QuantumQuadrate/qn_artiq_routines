...
class FORTPolarizationStabilizer:
    ...
    def __init__(self, experiment, initialize_to_home = False, search_start_from_scratch = False, reduce_sample_pts = False):
        ...
        self.exp.set_dataset("FORT_MM_monitor", [], broadcast=True)
        self.exp.set_dataset("FORT_APD_monitor", [], broadcast=True)
        self.exp.set_dataset("HWP_angle", [], broadcast=True)
        self.exp.set_dataset("QWP_angle", [], broadcast=True)
    ...
    @kernel
    def optimization_routine(self, tolerance_deg, full_range, sample_pts):
        """
        ...
        #     self.exp.laser_stabilizer.run()

        # self.dds_FORT.set(frequency=self.f_FORT, amplitude=self.stabilizer_FORT.amplitude)
        # self.dds_FORT.sw.on()  ### turns FORT on


        tolerance = float(tolerance_deg)   # rather than using fixed iteration, stop when step < tolerance
        full_range = float(full_range)  # Start with a full range
        sample_pts = int(sample_pts)  # Number of samples per iteration
        ...
        self.exp.set_dataset("best_852HWP_to_max", best_HWP, broadcast=True, persist=True)
        self.exp.set_dataset("best_852QWP_to_max", best_QWP, broadcast=True, persist=True)
        self.exp.set_dataset("best_852_power", best_power, broadcast=True, persist=True)


    # @kernel
    def run(self, tolerance_deg, full_range, sample_pts, best_HWP, best_QWP):
        """
        ...
        # self.optimization_routine(tolerance_deg=tolerance_deg, full_range=full_range, sample_pts=sample_pts)
        ...
        tolerance = float(tolerance_deg)   # rather than using fixed iteration, stop when step < tolerance
        full_range = int(full_range)  # Start with a full range
        sample_pts = int(sample_pts)  # Number of samples per iteration
        ...
        if self.search_start_from_scratch:
            ...
            best_HWP, best_QWP, best_power = best_HWP, best_QWP, 0.0
        previous_hwp, previous_qwp, previous_power = 0.0, 0.0, 0.0
