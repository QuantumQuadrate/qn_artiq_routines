...
#     y = p["intercept"] +  p["contrast"] / 2 * (
#             -1 * np.exp(-shifted_t / p["tau_decay"]) * np.cos(2 * np.pi / p["t_period"] * shifted_t) + 1)
#
#     # return y
...
#     ["t_period", "t_dead", "contrast", "intercept", "tau_decay"],
#     fitting_function,
...
def fitting_function(x, p):
    y_upper = 0.0
    shifted_t = (x - p["t_dead"])
    y = p["y_lower"] + (p["y_lower"] - y_upper) / 2 * (
        np.exp(-shifted_t / p["tau_decay"]) * -1 *
