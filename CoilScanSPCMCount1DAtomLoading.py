"""
This code scans one of the coils to see the sensitivity of the
...
class CoilScanSPCMCount1DAtomLoading(EnvExperiment):
    ...
    def update_coil_values(self):
        ...
        self.coil_V0 = [self.AZ_bottom_volts_MOT, self.AZ_top_volts_MOT, self.AX_volts_MOT, self.AY_volts_MOT][self.coil_index]
        self.coil_volts = [self.AZ_bottom_volts_MOT, self.AZ_top_volts_MOT, self.AX_volts_MOT, self.AY_volts_MOT]

        if self.differential_scan:
            for i in range(4):
                self.coil_V_array[i] += self.coil_volts[i]
    ...
    def scan_experiment(self):
        ...
        print("estimated duration of scan:",
              self.V_steps * (
                      self.t_MOT_loading + self.t_SPCM_exposure) / 60,
              "minutes")
