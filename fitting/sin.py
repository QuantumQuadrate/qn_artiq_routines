...
def fitting_function(x, p):
    ...
    y = p['a'] * np.sin(2 * np.pi * (x - p['x0']) / p['period'])
    y += p['y0']

    return y


# Sine with 'dumb' initialiser
sin = FitBase.FitBase(['x0', 'y0', 'a', 'period'],
                      fitting_function,
...
def parameter_initialiser_fft(x, y, p):
    ...
    p['period'] = period_sample / (i_max + 1.0)
...
sin_fft = FitBase.FitBase(['x0', 'y0', 'a', 'period'],
                          fitting_function,
